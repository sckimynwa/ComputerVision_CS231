...
def batchnorm_backward(dout, cache):
    ...
    dnorm_x = dout * gamma
    dsample_var = np.sum(dnorm_x * (x - sample_mean) * -0.5 * np.power(sample_var + eps, -1.5), axis = 0, keepdims = True)
    dsample_mean = np.sum(dnorm_x * (-1) / np.sqrt(sample_var + eps), axis = 0, keepdims = True) + dsample_var * np.mean( -2 * ( x - sample_mean), axis = 0, keepdims = True)
    
    dx = dnorm_x / np.sqrt(sample_var + eps) + dsample_var * 2 * ( x - sample_mean) / N + dsample_mean / N
...
def batchnorm_backward_alt(dout, cache):
    ...
    ## this is the code
    dxhat = dout * gamma 
    dx = (1./N) * (1./sample_var) *(N*dxhat - np.sum(dxhat, axis=0))
...
def conv_backward_naive(dout, cache):
    ...
    x_pad = np.pad(x, pad_width=xpad, mode='constant', constant_values=0)
    ...
    for i in range(N):
      for h_out in range(H_out):
        for w_out in range(W_out):
          out[i, :, h_out, w_out] = np.max(x[i, :, h_out*stride:h_out*stride+pool_height, 
        ...
        for c in range(C):
          for h in range(H_out):
            for w in range(W_out):
              ind = np.unravel_index(np.argmax(x[i, c, h*stride:h*stride+pool_height, 
              w*stride:w*stride+pool_width], axis=None), (pool_height, pool_width))

              dx[i, c, h*stride:h*stride+pool_height, w*stride:w*stride+pool_width][ind] = \
                dout[i, c, h, w]
